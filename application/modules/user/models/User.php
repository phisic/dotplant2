...
#modelsuse app\modules\shop\models\Customer;
...
class User extends ActiveRecord implements IdentityInterface
{
	...
	{
		return [
			'signup' => ['username', 'email', 'password', 'name', 'offname'],
			'resetPassword' => ['password'],
			'requestPasswordResetToken' => ['email'],
			'registerService' => ['email', 'name', 'offname'],
			'updateProfile' => ['email', 'name', 'offname'],
			'completeRegistration' => ['name', 'offname', 'username'],
			'changePassword' => ['password', 'newPassword', 'confirmPassword'],
			// admin
			'search' => ['id', 'username', 'email', 'status', 'create_time', 'name', 'offname'],
			'admin' => ['username', 'status', 'email', 'password', 'name', 'offname'],
			'adminSignup' => ['username', 'status', 'email', 'password', 'name', 'offname'],
		];
	}
	...
	public function attributeLabels()
	{
		return [
			...
			'name' => Yii::t('app', 'Name'),
			'offname' => Yii::t('app', 'Partner name'),
	...
	public static function findIdentity($id)
	{
		...
		if (is_numeric($id))
		{
			...
			if ($model === false)
			{
				...
				{
					Yii::$app->cache->set(
							"User:$id", $model, 3600, new TagDependency([
						'tags' => [
	...
	{
		$name = trim($this->name . ' ' . $this->offname);
